import { AlertTriangle, ArrowRight, BookOpen, CheckCircle2, RefreshCw, X } from 'lucide-react';
...
export function RevisionQueue() {
  ...
  const loadRecommendations = useCallback(async (showLoading = false) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/recommendations`, { cache: 'no-store' });
  ...
  useEffect(() => {
    ...
    const refresh = () => void loadRecommendations();
    ...
    <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">Live recommendations based on saved mastery scores.</p>
        </div>
        <button type="button" onClick={() => void loadRecommendations(true)} disabled={isRefreshing} aria-label="Refresh revision recommendations" className="rounded-xl bg-slate-800 p-2 text-cyan-300 transition hover:bg-slate-700 disabled:opacity-60">
      ...
      {selectedTopic ? <div className="mt-5 rounded-xl border border-cyan-500/30 bg-cyan-500/5 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-[0.18em] text-cyan-400">Selected revision</p><h3 className="mt-1 text-lg font-semibold text-white">{selectedTopic.topic}</h3><p className="mt-1 text-sm text-slate-400">The practice generator is ready below. Generate a short retry set, then refresh this queue after answering.</p></div><button type="button" onClick={() => setSelectedTopic(null)} aria-label="Close selected revision" className="rounded-lg p-1 text-slate-400 hover:bg-slate-800 hover:text-white"><X className="h-4 w-4" /></button></div><div className="mt-3 flex items-center gap-2 text-sm text-emerald-300"><CheckCircle2 className="h-4 w-4" />Previous mastery: {selectedTopic.masteryPercent}%</div></div> : null}
    </section>
  );
}
