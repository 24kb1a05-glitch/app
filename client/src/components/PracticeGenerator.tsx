    ...
    <section className="grid gap-6 lg:grid-cols-[1.4fr_0.9fr]">
      <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <button type="button" onClick={handleGenerate} disabled={isGenerating || isLoadingFiles} className="rounded-full bg-cyan-500 px-4 py-2 text-sm font-semibold text-slate-950 hover:bg-cyan-400 disabled:cursor-not-allowed disabled:opacity-60">
        ...
        <label className="mb-4 block rounded-xl border border-slate-800 bg-slate-950/70 p-3">
          ...
          <select value={selectedFileId} onChange={(event) => setSelectedFileId(event.target.value)} className="w-full bg-transparent text-sm text-white outline-none" disabled={isLoadingFiles}>
        ...
        <div className="mt-5 space-y-4">
          {questions.map((item) => <div key={item.id} className="rounded-xl border border-slate-800 bg-slate-950/70 p-4"><div className="mb-2 flex items-center justify-between gap-3"><p className="font-medium text-white">{item.question}</p><span className="rounded-full bg-cyan-500/15 px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-cyan-300">{item.source}</span></div><p className="text-sm text-slate-400">Answer: {item.answer}</p>{item.explanation ? <p className="mt-2 text-xs text-slate-500">{item.explanation}</p> : null}</div>)}
        </div>
      </div>
