                ...
                {recentMaterials.map((item, index) => (
                  <div key={item.name} className="rounded-xl border border-slate-800 bg-slate-950/70 p-3">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className={`rounded-full px-2 py-1 text-[10px] font-semibold uppercase tracking-wide ${item.status === 'Ready' ? 'bg-emerald-500/15 text-emerald-300' : 'bg-amber-500/15 text-amber-300'}`}>
                        {item.status}
                      </span>
                    </div>
            ...
            <aside className="space-y-5 rounded-2xl border border-slate-800 bg-slate-900 p-5">
              ...
              <div className="space-y-2">
                {quickActions.map((action) => (
                  <button key={action.label} className="flex w-full items-center justify-between rounded-xl border border-slate-800 bg-slate-950/70 px-3 py-2 text-left text-sm text-slate-200 transition hover:border-cyan-500 hover:text-white">
